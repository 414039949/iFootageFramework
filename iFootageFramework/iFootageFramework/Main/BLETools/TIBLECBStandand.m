...
//    NSLog(@"广播模式%@", p);
//    NSLog(@"serviceUUID%d", serviceUUID);
    ...
    CBService *service = [self findServiceFromUUID:su p:p];
    if (!service) {
//        NSLog(@"广播模式Could not find service with UUID %s on peripheral with UUID %s\r\n",[self CBUUIDToString:su],[self UUIDToString:(__bridge CFUUIDRef)(p.services[0])]);
        return;
    }
    CBCharacteristic *characteristic = [self findCharacteristicFromUUID:cu service:service];
    if (!characteristic) {
//        NSLog(@"广播模式Could not find characteristic with UUID %s on service with UUID %s on peripheral with UUID %s\r\n",[self CBUUIDToString:cu],[self CBUUIDToString:su],[self UUIDToString:(__bridge CFUUIDRef)(p.services[0])]);
...
//        NSLog(@"%d  |  %s\r\n",i,CFStringGetCStringPtr(s, 0));
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName: @"BLEDEVICEFOUND" object: p];
    }
}
...
//    NSLog(@"timeruuid%@", UUID);
//    NSLog(@"timerservice%@", service);
//    NSLog(@"timercount%ld", service.characteristics.count);
...
//        NSLog(@"active = %@", self.activeCharacteristics);
//        NSLog(@"New characteristics, adding ... characteristic UUID %s",[self CBUUIDToString:c.UUID]);
    }
}
...
//    NSLog(@"000000%@%@", peripheral.name, peripheral.identifier);
...
//    NSLog(@"RSSI : %d\r\n",[RSSI intValue]);
    ...
    NSLog(@" RSSI%@", RSSI);
    NSLog(@"\r\nDATA===================%@", advertisementData[kCBADVDATAMANUFACTURERDATA]);
    if ([advertisementData[kCBADVDATAMANUFACTURERDATA] length] < 18) {
        return;

    }else{
        
    typeStr = [self getPrefixTypeString:[NSString stringWithFormat:@"%@", advertisementData[kCBADVDATAMANUFACTURERDATA]]];
    
    }
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:S1A3orMiniID] isEqualToString:@"S1A3"] == YES) {
        if ([typeStr isEqualToString:S1A3_X2INDETIFER] == YES) {
...
//
    ...
    //列表为空，第一次发现新设备
        ...
        if([[NSString stringWithFormat:@"%@", peripheral.name] rangeOfString:IFootagePreFIX].location !=NSNotFound){
            
            if (pathDict) {
                NSString * nameStr = pathDict[[NSString stringWithFormat:@"%@", peripheral.identifier]][[pathDict[[NSString stringWithFormat:@"%@", peripheral.identifier]] allKeys][0]];
                NSDictionary * dict;
                
                if (nameStr) {
                    dict  = @{[NSString stringWithFormat:@"%@", peripheral.identifier]:@{[NSString stringWithFormat:@"%@", advertisementData[kCBADVDATAMANUFACTURERDATA]]:nameStr}};
                }else{
                    dict = @{[NSString stringWithFormat:@"%@", peripheral.identifier]:@{[NSString stringWithFormat:@"%@", advertisementData[kCBADVDATAMANUFACTURERDATA]]:[NSString stringWithFormat:@"%@", peripheral.name]}};
                }
                
                [pathDict addEntriesFromDictionary:dict];
                [pathDict writeToFile:path atomically:YES];
                
            }else{
                 NSDictionary * dict = @{[NSString stringWithFormat:@"%@", peripheral.identifier]:@{[NSString stringWithFormat:@"%@", advertisementData[kCBADVDATAMANUFACTURERDATA]]:[NSString stringWithFormat:@"%@", peripheral.name]}};
...
//            if ([self UUIDSAreEqual:p.UUID u2:peripheral.UUID]) {
              ...
              if (p.identifier == peripheral.identifier) {
                  if([peripheral.name rangeOfString:IFootagePreFIX].location !=NSNotFound){
                ...
                //发送外围设备的序号，以及RSSI通知
                  ...
                  if(isScan)
                  {
                     [[NSNotificationCenter defaultCenter] postNotificationName: @"BLEDEVICEWITHRSSIFOUND" object: rssiArray];
                  }
                return;
                  }
            }
...
//            NSLog(@"ifootage === == === %@", peripheral.name);
...
#warning 崩溃 定位- ------ 
//    [_showDelegate freeconnectDelegate];
//    NSLog(@"//step_2 didDiscoverPeripheral\r\n");
    //发送外围设备的序号，以及RSSI通知
    rssiArray = [NSArray arrayWithObjects:[NSNumber numberWithInt:i],RSSI, nil];
//    NSLog(@"RSSIArray%@",rssiArray);
...
//    NSLog(@"22222%@", peripheral);
//    
//    if (self.peripherals) {
//        
//        NSLog(@"GUO%@", self.peripherals);
//        for (CBPeripheral * cb in self.peripherals) {
//            NSLog(@"11111%@", cb);
//            if ([cb.identifier isEqual:peripheral.identifier] == YES) {
//                NSLog(@"333333");
//            }
//        }
//    }
    
//    NSLog(@"adver广播数据 == %@", advertisementData);
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error{
    ...
    [ReceiveView sharedInstance].slideisConnect_24G = 0;
    [ReceiveView sharedInstance].X2isConnect_24G = 0;
    [ReceiveView sharedInstance].S1A3_X2_2_4GAddress = 0;
    [ReceiveView sharedInstance].S1A3_X2_2_4GAddress_TimeStamp = 0;
    [ReceiveView sharedInstance].S1A3_S1_2_4GAddress = 0;
    [ReceiveView sharedInstance].S1A3_S1_2_4GAddress_TimeStamp = 0;

    
    [_showDelegate disconnectDelegate:peripheral];
    NSLog(@">>>外设连接断开连接 %@%@: %@\n", [peripheral name], [peripheral identifier], [error localizedDescription]);
    
//    NSLog(@"1 %@", self.peripherals);
    NSMutableArray * arraytemp = [[NSMutableArray alloc]initWithArray:self.peripherals];
    NSArray * array = [NSArray arrayWithArray:arraytemp];
    
    if ([error localizedDescription]) {
//        NSLog(@"3 %@", self.peripherals);
        for (CBPeripheral * cb in array) {
//            NSLog(@"4 %@", self.peripherals);
            
            if ([cb.identifier isEqual:peripheral.identifier] == YES) {
                [arraytemp removeObject:cb];
            }
//            NSLog(@"5 %@", self.peripherals);
        }
    }
    self.peripherals = arraytemp;
//    NSLog(@"2 %@", self.peripherals);
    [[NSNotificationCenter defaultCenter]postNotificationName:@"StatusViewNotificationMethod" object:nil];
...
-(NSString *)getUUIDString
{
    ...
    NSString *auuid = [[NSString alloc]initWithFormat:@"%@", activePeripheral.identifier];
    if (auuid.length >= 36) {
